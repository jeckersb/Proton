  ...
  class OutgoingMessageHandler < Qpid::Proton::BaseHandler
    ...
    def on_delivery(event)
      ...
      if delivery.link.sender? && delivery.updated?
        ...
        elsif delivery.rejected?
          self.on_rejected(event)
        elsif delivery.released? || delivery.modified?
    ...
    def on_sendable(event)
      dispatch(@delegate, :on_sendable, event) unless @delegate.nil?
    end
    ...
    def on_accepted(event)
      dispatch(@delegate, :on_accepted, event) unless @delegate.nil?
    end
    ...
    def on_rejected(event)
      dispatch(@delegate, :on_rejected, event) unless @delegate.nil?
    end
    ...
    def on_released(event)
      dispatch(@delegate, :on_released, event) unless @delegate.nil?
    end
    ...
    def on_settled(event)
      dispatch(@delegate, :on_settled, event) unless @dispatch.nil?
    end
